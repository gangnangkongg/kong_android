...
class MyViewModel: ViewModel() {
    private val _myData = MutableLiveData<HomeResponse>()
    val myData: LiveData<HomeResponse> = _myData
    ...
    // 내 데이터 요청
    fun fetchMyData(token: String) {
        retrofitService.fetchHomeData(token).enqueue(object : Callback<HomeResponse> {
            override fun onResponse(call: Call<HomeResponse>, response: Response<HomeResponse>) {
                if (response.isSuccessful) {
                    _myData.value = response.body()
                } else {
                    Log.e("MyViewModel", "My data fetch failed: ${response.errorBody()}")
                }
            }

            override fun onFailure(call: Call<HomeResponse>, t: Throwable) {
                Log.e("MyViewModel", "My data fetch error: ${t.message}")
            }
        })
    }
        ...
        retrofitService.fetchAnalysisData(token).enqueue(object : Callback<AnalysisResponse> {
            override fun onResponse(call: Call<AnalysisResponse>, response: Response<AnalysisResponse>) {
                if (response.isSuccessful) {
                    ...
                    Log.e("MyViewModel", "Analysis data fetch failed: ${response.errorBody()}")
                }
            }

            override fun onFailure(call: Call<AnalysisResponse>, t: Throwable) {
                Log.e("MyViewModel", "Analysis data fetch error: ${t.message}")
            }
        })
    }
